...
"""
PART 3 (BINARY ONLY) — FEATURE-VECTOR-ONLY BASELINES + XGBOOST/LIGHTGBM + STAGE A/B SPLIT

You have TWO feature files:
  - STAGE_A_PATH: 90% train + 10% validation  (used for training + threshold tuning)
  - STAGE_B_PATH: labeled test set             (used ONLY for final evaluation)

What this script does:
  1) Loads Stage A + Stage B from JSONL feature files (numeric features only).
  2) For each feature subset (4 configs):
       - trains models on Stage A train
       - tunes decision threshold on Stage A validation to maximize F1
       - evaluates once on Stage B test
  3) Exports:
       - results_binary/tables/binary_results.csv  (Accuracy/Precision/Recall/F1)
       - results_binary/figures/binary_f1_bar_<Subset>.pdf  (4 bar charts)
       - results_binary/figures/binary_confusion_best.pdf   (best overall run)
...
Models included:
  ...
  - SVM-RBF (Calibrated)
  - Sklearn MLP
  - Torch TabMLP (deep tabular)
  - FeatBERT (Transformer over feature tokens; no text)
